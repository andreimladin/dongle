...
# NOT run in CI — this is a release maintainer's manual/local step.
#
# No feed coordinates are hardcoded here: this script clones the plugin
# index fresh into a temp dir on every run and shells out to tools/resolve
# (a small build-time-only Go helper in this repo — not a dongle
# subcommand) to read each plugin's Azure Artifacts feed
