...
// release binary (see configs/build.yaml, scripts/fetch-embedded.sh, and
// scripts/build-binary.sh) into the normal plugin store on first run.
// Everything the //go:embed directive needs — the directive itself and the
// staged embedded/ payload — lives here because go:embed paths are relative
// to the source file and can't reach outside this package with "../".
package bootstrap
...
// embeddedFS holds whatever scripts/fetch-embedded.sh staged into
...
// scripts/fetch-embedded.sh from configs/build.yaml.
...
// configs/build.yaml, scripts/fetch-embedded.sh, and
// scripts/build-binary.sh) into the plugin store on first run only, then
// records that in state so it never runs again. It is a no-op when nothing
// was staged — either a plain `go build -tags embed` with an empty
// internal/bootstrap/embedded, or a run after the bootstrap
// flag is already set.
