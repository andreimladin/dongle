...
// release binary (see build.yaml and scripts/build.sh) into the normal
// plugin store on first run. Everything the //go:embed directive needs —
// the directive itself and the staged embedded/ payload — lives here
// because go:embed paths are relative to the source file and can't reach
// outside this package with "../".
package bootstrap
...
// scripts/build.sh's fetch_embedded from build.yaml.
...
// build.yaml and scripts/build.sh) into the plugin store on first run only,
// then records that in state so it never runs again. It is a no-op when
// nothing was staged — either a plain `go build -tags embed` with an empty
