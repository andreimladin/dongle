...
- **Credential store** (the "5a" stopping point): `login`/`logout` and a `0600`
  credential store. Dispatch injects the context contract
  (`DONGLE_VERSION`/`DONGLE_PROTOCOL`/`DONGLE_PLUGIN_NAME`).
...
Not wired in yet (the seams are in place):

- **Credential injection into the plugin** (the "5b" step): turning a plugin's
  declared `auth` into an injection (`injectAs: file` → temp `0600` file torn
  down after exit; `env` → env var) and having dispatch pass it to the exec.
  `auth.token()` is the seam it builds on.
...
- **env** — today: `DONGLE_VERSION`, `DONGLE_PROTOCOL`, `DONGLE_PLUGIN_NAME`.
  The 5b step adds `DONGLE_INVOCATION_ID`, `DONGLE_OUTPUT`,
  `DONGLE_CREDENTIALS_FILE`, and any `injectAs:"env"` credentials. (The Go SDK
  already reads the full set; the extra vars are simply empty until 5b lands.)
