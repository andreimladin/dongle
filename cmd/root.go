...
// scripts/fetch-embedded.sh, scripts/build-binary.sh, and configs/build.yaml
// — the human-edited source of truth those scripts bake these values from).
// A plain `go build ./cmd` leaves them at these defaults: hostVersion "dev",
// no index URL (DONGLE_INDEX_URL is then required to use `dongle
