...
| `internal/builtins` | `dongle search/install/remove/upgrade/sync/support` + the `--version` report (which also lists installed plugins) |
...
- **`plugin install` copies from a local directory** (`internal/builtins.Install`)
